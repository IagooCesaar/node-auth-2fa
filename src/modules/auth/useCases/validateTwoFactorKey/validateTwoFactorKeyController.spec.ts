...
describe("Validate Two Factor Key Controller", () => {
  beforeAll(async () => {
  ...
  it("Should be able to validate a Two-Factor Key", async () => {
    ...
    const response = await request(app).post("/sessions/two-factor").send({
      totp_code,
      temporaryToken,
    });
